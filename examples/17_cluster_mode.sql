...
CREATE SERVER redis_cluster
    ...
    OPTIONS (
        host_port '127.0.0.1:7001,127.0.0.1:7002,127.0.0.1:7003'
    );

-- You can also use cluster_mode explicitly with a single entry point:
CREATE SERVER redis_cluster_single_entry
    FOREIGN DATA WRAPPER redis_wrapper
    OPTIONS (
...
-- NOTE: SELECT * (full SCAN) and LIKE are NOT supported in cluster mode
...
INSERT INTO cluster_cache_ttl VALUES ('cluster:ttlcache:warm', '{"data":"normal-expiry"}', 300);
...
-- ============================================================
-- Data written via one server entry point is visible from another,
-- proving both connections resolve to the same cluster.

CREATE FOREIGN TABLE cluster_verify_write (value text)
    SERVER redis_cluster
    OPTIONS (
        table_type 'string',
        table_key_prefix 'cluster:verify:cross-node'
    );

CREATE FOREIGN TABLE cluster_verify_read (value text)
    SERVER redis_cluster_single_entry
    OPTIONS (
        table_type 'string',
        table_key_prefix 'cluster:verify:cross-node'
    );

INSERT INTO cluster_verify_write VALUES ('written-via-multi-node-visible-everywhere');
SELECT * FROM cluster_verify_read;
...
-- NOTE: Multi-key TRUNCATE is NOT supported in cluster mode:
-- CREATE FOREIGN TABLE cluster_trunc_multi (key text, value text)
--     SERVER redis_cluster
--     OPTIONS (table_type 'string', table_key_prefix 'cluster:trunc:tmp:*');
-- TRUNCATE cluster_trunc_multi;  -- ERROR: SCAN cannot route in cluster

-- ============================================================
-- Part 8: EXPLAIN on cluster tables
-- ============================================================

CREATE FOREIGN TABLE cluster_explain_zset (member text, score numeric)
    SERVER redis_cluster
    OPTIONS (
        table_type 'zset',
        table_key_prefix 'cluster:explain:scores'
    );

INSERT INTO cluster_explain_zset VALUES ('x', 10);
INSERT INTO cluster_explain_zset VALUES ('y', 20);
INSERT INTO cluster_explain_zset VALUES ('z', 30);

EXPLAIN (VERBOSE) SELECT * FROM cluster_explain_zset;
EXPLAIN (VERBOSE) SELECT * FROM cluster_explain_zset WHERE score >= 15;
EXPLAIN (VERBOSE) SELECT * FROM cluster_explain_zset WHERE member = 'y';
...
DROP FOREIGN TABLE IF EXISTS cluster_explain_zset;
DROP SERVER IF EXISTS redis_cluster;
DROP SERVER IF EXISTS redis_cluster_single_entry;
